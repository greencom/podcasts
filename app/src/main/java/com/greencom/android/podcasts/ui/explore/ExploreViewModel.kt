...
class ExploreViewModel @Inject constructor(private val repository: Repository) : ViewModel() {
    ...
    /** Backing property for a [message]. Use [setMessage] to set a new value. */
    ...
    fun getBestPodcastsFlow(genreId: Int): Flow<State> =
        repository.getBestPodcastsFlow(genreId)

    /** Update the best podcasts for a given genre ID. */
    fun updateBestPodcastsFlow(genreId: Int) {
        viewModelScope.launch {
            when (repository.updateBestPodcastsFlow(genreId)) {
                is State.Success<*> -> {
     ...
     * the database.
     */
    fun fetchBestPodcastsFlow(genreId: Int) {
        viewModelScope.launch {
    ...
    fun updateSubscriptionFlow(podcast: Podcast, subscribed: Boolean) = viewModelScope.launch {
        repository.updateSubscriptionFlow(podcast, subscribed)
    }
